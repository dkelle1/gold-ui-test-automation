 ...
 * through checkout) and would make positive-path tests flaky if assigned interchangeably. They are
 * instead targeted directly by a test that opts in with `test.use({ userOverride: '<username>' })` -
 * the Playwright Test equivalent of the Cucumber sibling's `@user:<username>` tag.
 */
...
/** All six accounts, keyed by username, for `userOverride` lookup. */
